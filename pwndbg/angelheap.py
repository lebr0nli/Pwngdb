...
class Malloc_bp_ret(gdb.FinishBreakpoint):
    ...
    def stop(self):
        ...
        chunk["size"] = (
            int(gdb.execute(cmd, to_string=True).split(":")[1].strip(), 16)
            & 0xFFFFFFFFFFFFFFF8
        )
        ...
        if hex(chunk["addr"]) in freerecord:
            ...
            if chunk["size"] != freechunk["size"]:
                ...
                freerecord[hex(splitchunk["addr"])] = copy.deepcopy(
                    (
                        splitchunk["addr"],
                        splitchunk["addr"] + splitchunk["size"],
                        splitchunk,
                    )
                )
...
class Malloc_Bp_handler(gdb.Breakpoint):
    def stop(self):
        ...
        if arch == "x86-64":
            reg = "$rsi"
            arg = int(
                gdb.execute("info register " + reg, to_string=True).split()[1].strip(),
                16,
            )
        else:
...
class Free_Bp_handler(gdb.Breakpoint):
    def stop(self):
        ...
        if arch == "x86-64":
            ...
            result = (
                int(
                    gdb.execute("info register " + reg, to_string=True)
                    .split()[1]
                    .strip(),
                    16,
                )
        ...
        if overlap and status == "error":
            if DEBUG:
                ...
                print(
                    "\033[31mdouble free detected !! (0x%x)\033[37m" % overlap["addr"]
                )
...
def get_libc_version():
    ...
    try:
        libc_version = float(
            gdb.execute("x/s __libc_version", to_string=True).split()[2].strip('"')
        )
    except:
    ...
    if data:
        for key, (start, end, chunk) in data.items():
            if (
                ...
                or ((addr + size) > start and (addr + size) < end)
                or ((addr < start) and ((addr + size) >= end))
            ):
                return chunk, "error"
    else:
...
def get_all_threads():
    ...
    all_threads = [
        ...
        for line in gdb.execute(cmd, to_string=True).replace("*", "").split("\n")[1:-1]
    ]
    return all_threads
...
def get_tcache():
    ...
    try:
        tcache_max_bin = int(
            gdb.execute("x/" + word + " &mp_.tcache_bins", to_string=True)
            .split(":")[1]
            .strip(),
            16,
        )
        try:
            tcache_enable = True
            tcache = int(
                gdb.execute("x/" + word + "&tcache", to_string=True)
                .split(":")[1]
                .strip(),
                16,
            )
            tps_size = int(
                gdb.execute("p sizeof(*tcache)", to_string=True).split("=")[1].strip()
            )
            if capsize == 4:
                if tps_size > 0x140:
    ...
    if chunk["addr"] == chunkhead["addr"]:  # no chunk in the bin
        if chunkhead["addr"] > arena:
            ...
            try:
                ...
                chunk["overlap"] = is_overlap
                chunk["memerror"] = (
                    "\033[31mbad fd (" + hex(chunk["addr"]) + ")\033[37m"
                )
            except:
                chunk["memerror"] = "invaild memory"
        ...
        try:
            ...
            if bk_fd != chunkhead["addr"]:
                chunkhead[
                    "memerror"
                ] = "\033[31mdoubly linked list corruption {0} != {1} and \033[36m{2}\033[31m is broken".format(
        ...
        while chunk["addr"] != chunkhead["addr"]:
            ...
            try:
                ...
                if chunk["addr"] != fd_bk:
                    chunk[
                        "memerror"
                    ] = "\033[31mdoubly linked list corruption {0} != {1} and \033[36m{2}\033[31m or \033[36m{3}\033[31m is broken".format(
...
def get_smallbin(arena=None):
    ...
    for size in range(capsize * 4, max_smallbin_size, cursize * 2):
        ...
        cmd = (
            "x/" + word + hex(bins_addr + idx * capsize * 2)
        )  # calc the smallbin index
        chunkhead["addr"] = int(
            gdb.execute(cmd, to_string=True).split(":")[1].strip(), 16
        )
        try:
...
def get_largebin(arena=None):
    ...
    for idx in range(64, 128):
        chunkhead = {}
        cmd = (
            "x/" + word + hex(bins_addr + idx * capsize * 2 - 2 * capsize)
        )  # calc the largbin index
        chunkhead["addr"] = int(
            gdb.execute(cmd, to_string=True).split(":")[1].strip(), 16
        )
        try:
            ...
            if size <= capsize * 0x10:  # fastbin
                ...
                if not prev_inuse:
                    ...
                    if prev_size != prev_chunk_size:
                        print(
                            "\033[32mFreeable :\033[1;31m False -> p->size(0x%x) != next->prevsize(0x%x) \033[37m"
...
def get_heapbase():
    if (main_arena and not enable_thread) or thread_arena == main_arena:
        heapbase = int(
            gdb.execute("x/" + word + " &mp_.sbrk_base", to_string=True)
            .split(":")[1]
            .strip(),
            16,
        )
        ...
        arena_size = int(
            gdb.execute("p sizeof(main_arena)", to_string=True).split("=")[1].strip(),
            16,
        )
            ...
            if chunkaddr in fastchunk:
                ...
                if (size & 0xFFFFFFFFFFFFFFF8) <= 0x80:
                    ...
                    cmd = (
                        "x/"
                        + word
                        + hex(next_chunk + (nextsize & 0xFFFFFFFFFFFFFFF8) + capsize)
                    )
    ...
    for i, bins in enumerate(fastbin):
        ...
        for chunk in bins:
            if "memerror" in chunk:
                print(
                    "\033[31m0x%x (%s)\033[37m" % (chunk["addr"], chunk["memerror"]),
                    end="",
                )
...
def put_tcache():
    ...
    for i, entry in enumerate(tcache_entry):
        ...
        if len(tcache_entry[i]) > 0:
            print(
                "\033[33;1m(0x%02x)   tcache_entry[%d]\033[32m(%d)\033[33;1m:\033[37m "
                % (cursize, i, tcache_count[i]),
                end="",
            )
        elif tcache_count[i] > 0:
            print(
                "\033[33;1m(0x%02x)   tcache_entry[%d]\033[31;1m(%d)\033[33;1m:\033[37m 0\n"
                % (cursize, i, tcache_count[i]),
                end="",
            )
        for chunk in entry:
            if "memerror" in chunk:
                print(
                    "\033[31m0x%x (%s)\033[37m"
                    % (chunk["addr"] + capsize * 2, chunk["memerror"]),
                    end="",
                )
            elif chunk["overlap"] and chunk["overlap"][0]:
                print(
...
def putheapinfo(arena=None):
    ...
    if "memerror" in top:
        ...
        print(
            "\033[35m %20s:\033[34m 0x%x \033[33m(size : 0x%x)\033[37m "
    ...
    for size, bins in smallbin.items():
        ...
        print(
            "\033[33m(0x%03x)  %s[%2d]:\033[37m " % (int(size, 16), "smallbin", idx),
            end="",
        )
        for chunk in bins:
            if "memerror" in chunk:
    ...
    if main_arena:
        try:
            ...
            while cur_arena != main_arena:
                ...
                cur_arena = int(
                    gdb.execute("x/" + word + hex(cur_arena + nextoff), to_string=True)
                    .split(":")[1]
                    .strip(),
                    16,
                )
...
def putheapinfoall():
    ...
    for thread_id in all_threads:
        if thread_id == cur_thread_id:
            print(
                "\033[33;1m"
                + ("  Thread " + str(thread_id) + "  ").center(50, "=")
                + "\033[0m",
                end="",
            )
        else:
...
def get_fake_fast(addr, size=None):
    if capsize == 0:
        arch = getarch()
    fast_max = int(
        gdb.execute("x/" + word + "&global_max_fast", to_string=True)
        .split(":")[1]
        .strip(),
        16,
    )
    ...
    if size:
        ...
        for fakechunk in chunk_list:
            if len(chunk_list) > 0:
                print(
                    "\033[1;33mfake chunk : \033[1;0m0x{:<12x}\033[1;33m  padding :\033[1;0m {:<8d}".format(
        ...
        for i in range(int(fast_max / (capsize * 2) - 1)):
            ...
            if len(chunk_list) > 0:
                ...
                for fakechunk in chunk_list:
                    print(
                        "\033[1;33mfake chunk :\033[1;0m 0x{:<12x}\033[1;33m  padding :\033[1;0m {:<8d}".format(
                            fakechunk[0], fakechunk[1]
                        )
                    )
